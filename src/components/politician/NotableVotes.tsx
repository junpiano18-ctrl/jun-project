...
const MINE_LABEL: Record<VoteResult, { icon: string; label: string; cls: string }> = {
  AGREE:    { icon: "✅", label: "찬성했어요", cls: "text-emerald-700" },
  DISAGREE: { icon: "❌", label: "반대했어요", cls: "text-red-700" },
  ABSTAIN:  { icon: "🟡", label: "기권했어요", cls: "text-amber-700" },
    ...
    <ul className="space-y-2.5">
      {votes.map((v) => {
        ...
        return (
          <li
            key={v.id}
            className="rounded-lg bg-amber-50 p-3 ring-1 ring-amber-200"
          >
            {/* 법안명 + 의안링크 */}
            {v.billUrl ? (
              <a
            ...
            {/* 전체 표결 분포 */}
            <div className="mt-2.5 overflow-hidden rounded-full bg-zinc-200">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
              <span className="text-emerald-700">찬성 {yes}명 ({yesPct}%)</span>
              <span className="text-zinc-300">·</span>
              <span className="text-red-700">반대 {no}명 ({noPct}%)</span>
              {blank > 0 && (
                <>
                  <span className="text-zinc-300">·</span>
                  <span className="text-amber-700">기권 {blank}명</span>
                </>
              )}
            </div>
            ...
            <p className="mt-2 flex items-center gap-1.5 text-sm font-semibold">
              ...
              <span className="text-zinc-500">이 의원:</span>
